...
Use navin as a Python library. The SDK gives you the same agent runtime used
...
- navin SDK runs an agent around a model.
...
Install and configure navin first. If you have not done that yet, follow the
...
`Navin.from_config()` reuses your normal `~/.navin/config.json` and
`~/.navin/workspace/`. Provider, model, tools, memory, and session behavior
...
`navin status` should show the config path, workspace path, active model or
...
```python
from navin import STREAM_EVENT_TEXT_DELTA
...
Save this as `sdk_demo.py` after `navin agent -m "Hello!"` works:
...
async def main() -> None:
    prompt = " ".join(sys.argv[1:]) or "Explain what navin is in one paragraph."
    ...
    async with Navin.from_config() as bot:
        ...
        async for event in bot.stream(prompt, session_key=session_key):
            if event.type == STREAM_EVENT_TEXT_DELTA:
                ...
                raise RuntimeError(event.error or "navin run failed")
...
workspace: /Users/alice/.navin/workspace
...
This script shows the usual production shape: create one `Navin`, choose a
...
| Concept | Meaning |
|---------|---------|
| `Navin` | The SDK object that owns one configured agent runtime. |
...
| Memory | Long-term memory files managed by navin. |
...
navin has two programming surfaces:

| Use | Choose | Why |
|-----|--------|-----|
| Python code running in the same process as navin | Python SDK | Direct access to `RunResult`, sessions, memory, runtime helpers, hooks, and stream events. |
...
process isolation, or need to call navin from a non-Python service.
...
async with Navin.from_config(workspace="/my/project") as bot:
    result = await bot.run("Explain the project structure")
```

Use a custom config when you run multiple navin instances or test an isolated
setup:

```python
async with Navin.from_config(
...
The config controls what navin may use. The workspace is where navin keeps
...
```python
bot = Navin.from_config(model="openai/gpt-4.1")
```
...
```python
bot = Navin.from_config(model_preset="fast")
...
provider and model work with `navin agent -m "Hello!"` first.
...
become navin session history. Ingesting a transcript does not call the model,
...
metrics, tracing, or output post-processing without modifying navin internals:

```python
from navin.agent import AgentHook, AgentHookContext
...
### `Navin.from_config(config_path=None, *, workspace=None, model=None, model_preset=None)`

Create a `Navin` instance from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.navin/config.json`. |
...
```python
from navin.agent import AgentHook
