...
Build a custom navin channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of navin (`python -m pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

navin discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `navin gateway` starts, it scans:

1. Built-in channels in `navin/channels/`
2. External packages registered under the `navin.channels` entry point group
...
```text
navin-channel-webhook/
├── navin_channel_webhook/
...
```python
# navin_channel_webhook/__init__.py
from navin_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# navin_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "navin-channel-webhook"
version = "0.1.0"
dependencies = ["navin-ai", "aiohttp"]

[project.entry-points."navin.channels"]
webhook = "navin_channel_webhook:WebhookChannel"
...
packages = ["navin_channel_webhook"]
```
...
navin plugins list      # verify the installed example plugin appears as "webhook"
navin onboard           # auto-adds default config for detected plugins
```

Edit `~/.navin/config.json`:
...
```bash
navin channels login <channel_name>
navin channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `navin onboard`. Override to declare your fields. |
...
Besides normal assistant text, navin can emit low-emphasis trace blocks. These are intended for UI affordances like status rows, collapsible "used tools" groups, or reasoning/thinking blocks. Platforms that do not have a good place for them can ignore them safely.
...
```python
from navin.bus.outbound_events import ProgressEvent
...
1. Define a Pydantic model inheriting from `navin.config.schema.Base`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `navin-channel-{name}` | `navin-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `navin_channel_{name}` | `navin_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/navin-channel-webhook
cd navin-channel-webhook
python -m pip install -e .
navin plugins list    # should show the installed example plugin as "webhook"
navin gateway         # test end-to-end
```
