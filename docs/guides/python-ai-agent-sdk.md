...
This guide shows when to use the Navin Python SDK instead of calling a model
...
- a Python script that creates a `Navin`
...
- Reuse one `Navin` instance for related work.
...
- If SDK code fails, first run `navin agent -m "Hello!"` in the same
  environment.
