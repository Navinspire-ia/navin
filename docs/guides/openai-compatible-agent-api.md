# How to Run an OpenAI-Compatible Agent API with navin

navin can expose a local OpenAI-compatible endpoint behind
`/v1/chat/completions`. This lets existing OpenAI-style clients talk to a
tool-using navin agent instead of a raw model.
...
- If `/v1/chat/completions` fails, test `navin agent -m "Hello!"` first.
