...
- one successful `navin agent` run
...
- If `curl /models` fails, fix the provider endpoint before changing navin.
- If navin says the model is unknown, check the model ID expected by the
  provider.
- If auth fails, confirm whether the provider wants Bearer auth and whether the
  key is present in the environment that starts navin.
