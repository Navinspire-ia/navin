...
Use this page when you want to understand navin before changing advanced settings. It explains the moving parts without requiring you to read the source first.

If you want source-file ownership and extension points, read [`architecture.md`](./architecture.md) after this page.

## Runtime Shape

navin has one small core loop and several ways to enter it:
...
| Channels | User-facing transports such as CLI, WebUI/WebSocket, Telegram, Discord, Slack, Email, Mattermost, and others |
...
The simplest path is `navin agent -m "Hello!"`: one inbound message goes through the agent loop and prints the reply in your terminal. The long-running path is `navin gateway`: channels receive messages from chat apps or the WebUI, publish them to the same agent loop, and send replies back to the originating channel.

## Config vs Workspace

The default instance lives under `~/.navin/`:

| Path | Meaning |
|---|---|
| `~/.navin/config.json` | Instance configuration: providers, model defaults, channels, tools, gateway, API, and runtime options |
| `~/.navin/workspace/` | Agent workspace: memory, sessions, heartbeat tasks, cron jobs, skills, and generated artifacts |

You can override both with command flags:

```bash
navin onboard --config ./bot-a/config.json --workspace ./bot-a/workspace
...
The config file controls what navin may use. The workspace is where navin keeps state for that instance.

## Config Format

`config.json` accepts both camelCase and snake_case keys. The docs use camelCase because navin writes config back to disk with camelCase aliases, for example `apiKey`, `modelPresets`, `intervalS`, and `maxToolResultChars`.

Most examples are partial snippets. Merge them into the existing file created by `navin onboard`; do not replace the whole file unless you want to reset the instance.
...
|---|---|---|
| CLI one-shot | `navin agent -m "..."` | First-run checks, scripts, and quick local questions |
| CLI interactive | `navin agent` | Terminal chat with persistent session history |
| Gateway | `navin gateway` | Chat apps, WebUI, heartbeat, Dream, and long-running service mode |
| OpenAI-compatible API | `navin serve` | Programmatic access through `/v1/chat/completions` |
| WebUI | `navin webui` | Prepare the local WebUI, start the gateway, and open the browser workbench |
...
1. If the active preset provider or implicit default provider is not `"auto"`, navin uses that provider.
2. If provider is `"auto"`, navin tries to infer the provider from the model name, configured API keys, local provider base URLs, or gateway providers.
...
When `navin gateway` starts, it runs workspace-scoped automations and
registers system jobs:

- `dream`, when `agents.defaults.dream.enabled` is true;
- `heartbeat`, when `gateway.heartbeat.enabled` is true.

Heartbeat reads `<workspace>/HEARTBEAT.md`. If the file has tasks under `## Active Tasks`, navin executes them and sends only useful/actionable results to the most recently active chat target. Routine "nothing changed" results are suppressed.
...
`navin trigger <id> "<message>"` when a local script or external service wants
navin to respond in that session. Webhook servers, third-party auth, and
event-to-message formatting stay outside navin. Trigger deliveries are stored
