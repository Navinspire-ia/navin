...
async def main():
    ...
    result = await VerifyTool(workspace=ROOT).execute(
        ...
        test_target='tests/test_code_autonomy.py',
    )
    Path('/tmp/navin-code-verify-result.txt').write_text(str(result), encoding='utf-8')
    print(result, flush=True)
    assert str(result).startswith('PASS -'), str(result)
    from navin.evals.agent_loop import main as eval_main
    code = await asyncio.to_thread(eval_main, ['--scoreboard', '--gate'])
    Path('/tmp/navin-code-eval-gate.txt').write_text('PASS' if code == 0 else 'FAIL')
    assert code == 0
