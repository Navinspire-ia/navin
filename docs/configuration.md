...
The generated `config.json` uses camelCase keys such as `apiKey` and `intervalS`. snake_case keys are also accepted for compatibility, but the docs prefer camelCase because that is what navin writes back to disk.
...
> [!NOTE]
> If your config file is older than the current schema, run `navin onboard --refresh`. navin adds missing default fields while preserving your existing values.
...
| Deploy the gateway | [`guides/deploy-navin-gateway.md`](./guides/deploy-navin-gateway.md) |
...
|---|---|---|---|
| Make the first model reply work | `providers.<name>.apiKey`, optional `providers.<name>.apiBase`, `modelPresets.<preset>`, `agents.defaults.modelPreset` | `navin status`, then `navin agent -m "Hello!"` | [Providers](#providers), [Model Presets](#model-presets) |
| Add fallback models | `modelPresets.<fallback>`, `agents.defaults.fallbackModels` | `navin status`, then a normal agent run | [Model Fallbacks](#model-fallbacks) |
| Keep secrets out of the config file | `${ENV_VAR}` placeholders inside any string value | Start navin from the same environment that sets the variable | [Environment Variables for Secrets](#environment-variables-for-secrets) |
| Open the bundled WebUI | `channels.websocket.enabled`, optional `channels.websocket.port`, `channels.websocket.tokenIssueSecret` | `navin webui` | [Channel Settings](#channel-settings), [WebSocket docs](./websocket.md) |
| Connect one chat app | `channels.<channel>.enabled`, channel credentials, optional pairing or `channels.<channel>.allowFrom` | `navin channels status`, then `navin gateway --verbose` | [Channel Settings](#channel-settings), [Chat Apps](./chat-apps.md) |
...
| Add external tools through MCP | `tools.mcpServers.<name>` | Start `navin gateway --verbose` and check startup/tool logs | [MCP](#mcp-model-context-protocol) |
...
| Tune request timeouts or process concurrency | `NAVIN_LLM_TIMEOUT_S`, `NAVIN_STREAM_IDLE_TIMEOUT_S`, `NAVIN_MAX_CONCURRENT_REQUESTS` | Start navin from the same environment and inspect startup/runtime logs | [Runtime Environment Variables](#runtime-environment-variables) |
| Run multiple isolated bots | separate `--config` and `--workspace` paths, plus distinct `gateway.port` or channel ports when processes run together | Use the same explicit paths with `navin status`, `agent`, `webui`, `gateway`, and `serve` | [Multiple Instances](./multiple-instances.md), [CLI Reference](./cli-reference.md) |
...
Any string value in `config.json` can use `${VAR_NAME}`. Resolution runs once at startup, in memory only — resolved values are never written back to disk, so editing config through `navin onboard` or the WebUI preserves the placeholder.

If a referenced variable is unset, navin fails fast at startup with `ValueError: Environment variable 'NAME' referenced in config is not set`.
...
Pick whatever fits your deployment — navin only reads `os.environ` at startup, so any mechanism that populates the process environment works.

**systemd** — use `EnvironmentFile=` in the service unit to load variables from a file that only the deploying user can read:

```ini
# /etc/systemd/system/navin.service (excerpt)
[Service]
EnvironmentFile=/home/youruser/navin_secrets.env
User=navin
ExecStart=...
```

```bash
# /home/youruser/navin_secrets.env (mode 600, owned by youruser)
...
```bash
docker run --rm --env-file=./navin.env \
...
op run --env-file=.env.tpl -- navin agent

# pass (passwordstore.org)
ANTHROPIC_API_KEY="$(pass show api/anthropic)" navin agent

# Bitwarden
ANTHROPIC_API_KEY="$(bw get password api/anthropic)" navin agent
```

## Runtime Environment Variables

These variables are process-level switches. Set them in the same terminal, service unit, container, or supervisor that starts navin.

### Runtime controls

| Variable | Default | Description |
|----------|---------|-------------|
| `NAVIN_MAX_CONCURRENT_REQUESTS` | `3` | Maximum concurrently running inbound agent requests. Must be an integer; set `0` or a negative value for unlimited. |
| `NAVIN_LLM_TIMEOUT_S` | `300` | Wall-clock timeout, in seconds. Ordinary requests use this value; streaming requests use the greater of 300 seconds or twice this value. Set `0` to disable. Sustained-goal turns bypass this wall-clock cap. |
| `NAVIN_STREAM_IDLE_TIMEOUT_S` | `90` | Streaming idle timeout, in seconds, used by streaming providers. Invalid or non-positive values are ignored; values above `3600` are clamped. |
| `NAVIN_OPENAI_COMPAT_TIMEOUT_S` | `120` | HTTP request timeout, in seconds, for OpenAI-compatible providers. Invalid or non-positive values are ignored. |
| `NAVIN_WORKSPACE_SANDBOX_ENFORCED` | unset | Marks that an external workspace sandbox is already enforced. Truthy values (`1`, `true`, `yes`, `on`, `enabled`) use `NAVIN_WORKSPACE_SANDBOX_PROVIDER` as the label; any other non-false value is treated as the provider name. |
| `NAVIN_WORKSPACE_SANDBOX_PROVIDER` | `unknown` | Display label for the external workspace sandbox when `NAVIN_WORKSPACE_SANDBOX_ENFORCED` is truthy, for example `macos_app_sandbox` or `bwrap`. |
| `NAVIN_SANDBOX_ENFORCED` | unset | Legacy compatibility alias for `NAVIN_WORKSPACE_SANDBOX_ENFORCED`. |
| `NAVIN_TMUX_SOCKET_DIR` | `${TMPDIR:-/tmp}/navin-tmux-sockets` | Socket directory used by the bundled `tmux` skill scripts. |

### Installer, build, and WebUI development

| Variable | Default | Description |
|----------|---------|-------------|
| `NAVIN_BIN_DIR` | `$HOME/.local/bin` | Installer launcher directory on macOS/Linux. |
| `NAVIN_VENV` | `$HOME/.navin/venv` | Managed virtual environment path used by the installer fallback. |
| `NAVIN_SKIP_WEBUI_BUILD` | unset | Set to `1` to skip bundling the WebUI during package builds. |
| `NAVIN_FORCE_WEBUI_BUILD` | unset | Set to `1` to rebuild the bundled WebUI even when `navin/web/dist/index.html` already exists. |
| `NAVIN_API_URL` | `http://127.0.0.1:8765` | Gateway target for the Vite WebUI dev server proxy. |

Internal variables such as `NAVIN_RESTART_*` and `NAVIN_PATH_*` are set by navin itself and are not a supported user configuration surface.

## Langfuse Observability

navin can trace OpenAI-compatible provider calls through Langfuse's OpenAI SDK wrapper. This is configured with environment variables, not `config.json`.

Install the optional package in the same Python environment that runs navin:

```bash
navin plugins enable langfuse
```

Set Langfuse credentials before starting `navin agent`, `navin gateway`, or `navin serve`:
...
When `LANGFUSE_SECRET_KEY` is set and the `langfuse` package is installed, navin uses `langfuse.openai.AsyncOpenAI` for OpenAI-compatible providers so model requests are sent to Langfuse in the background. If the secret key is set but `langfuse` is missing, navin logs a warning and falls back to the regular OpenAI client.
...
Tracing covers the providers that go through navin's OpenAI-compatible client path. Native providers that do not use that client may not produce Langfuse OpenAI-wrapper traces.
...
> - **MiniMax Coding Plan**: Exclusive discount links for the navin community: [Overseas](https://platform.minimax.io/subscribe/coding-plan?code=9txpdXw04g&source=link) · [Mainland China](https://platform.minimaxi.com/subscribe/token-plan?code=GILTJpMTqZ&source=link)
> - **MiniMax (Mainland China)**: If your API key is from MiniMax's mainland China platform (minimaxi.com), set `"apiBase": "https://api.minimaxi.com/v1"` in your minimax provider config.
> - **MiniMax thinking mode**: `providers.minimaxAnthropic` is the config block for `reasoningEffort` / thinking mode. MiniMax exposes that capability through its Anthropic-compatible endpoint, so navin keeps it as a separate provider instead of guessing MiniMax-specific thinking parameters on the generic OpenAI-compatible `minimax` endpoint. It uses the same `MINIMAX_API_KEY`. Default Anthropic-compatible base URL: `https://api.minimax.io/anthropic`; for mainland China use `https://api.minimaxi.com/anthropic`.
> - **Kimi Coding Plan**: Use `providers.kimiCoding` with `provider: "kimi_coding"` for Kimi's dedicated Anthropic Messages API endpoint. The endpoint requires a Claude-compatible `User-Agent`; navin sends `claude-code/0.1.0` by default, and you can override it with `extraHeaders.User-Agent` if your account requires a different value.
...
| `volcengine` | LLM (VolcEngine, pay-per-use) | [Coding Plan](https://www.volcengine.com/activity/codingplan?utm_campaign=navin&utm_content=navin&utm_medium=devrel&utm_source=OWO&utm_term=navin) · [volcengine.com](https://www.volcengine.com) |
| `volcengine_coding_plan` | LLM (VolcEngine Coding Plan subscription endpoint) | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=navin&utm_content=navin&utm_medium=devrel&utm_source=OWO&utm_term=navin) |
| `byteplus` | LLM (VolcEngine international, pay-per-use) | [Coding Plan](https://www.byteplus.com/en/activity/codingplan?utm_campaign=navin&utm_content=navin&utm_medium=devrel&utm_source=OWO&utm_term=navin) · [byteplus.com](https://www.byteplus.com) |
| `byteplus_coding_plan` | LLM (BytePlus Coding Plan subscription endpoint) | [byteplus.com](https://www.byteplus.com/en/activity/codingplan?utm_campaign=navin&utm_content=navin&utm_medium=devrel&utm_source=OWO&utm_term=navin) |
...
| `moonshot` | LLM (Moonshot/Kimi) | [platform.kimi.com](https://platform.kimi.com?aff=navin) |
| `kimi_coding` | LLM (Kimi Coding Plan, Anthropic Messages API) | [platform.kimi.com](https://platform.kimi.com?aff=navin) |
...
| `openai_codex` | LLM (Codex, OAuth) | `navin provider login openai-codex --set-main` |
| `github_copilot` | LLM (GitHub Copilot, OAuth) | `navin provider login github-copilot` |
| `qianfan` | LLM (Baidu Qianfan) | [cloud.baidu.com](https://cloud.baidu.com/doc/qianfan/s/Hmh4suq26) |

<details>
<summary><b>OpenAI</b></summary>

By default, OpenAI uses `apiType: "auto"`: navin calls Chat Completions normally and routes GPT-5/o-series or explicit `reasoningEffort` requests through the Responses API when useful. You can force a specific API surface:
...
> `apiBase` remains mandatory in both modes — it's your Azure resource endpoint and cannot be inferred. If neither `apiKey` is set nor `azure-identity` is installed, the provider raises a clear error pointing you at `navin plugins enable azure`.
...
```bash
navin plugins enable bedrock
```

> [!NOTE]
> If you configured Bedrock before `boto3` became an optional dependency, run
> `navin plugins enable bedrock` after upgrading. Otherwise the provider will
...
You can also set `providers.bedrock.apiKey` to a Bedrock API key; navin exports it as `AWS_BEARER_TOKEN_BEDROCK` for the AWS SDK.
...
Claude Opus 4.7 does not accept `temperature`, `top_p`, or `top_k`; navin omits `temperature` automatically for this model. If `reasoningEffort` is set to `low`, `medium`, `high`, `max`, or `adaptive`, navin sends Bedrock's adaptive thinking parameter.
...
Use Bedrock model IDs or inference profile IDs with a `bedrock/` prefix in navin config. navin removes the prefix before calling AWS.
...
Model-specific fields can be supplied with `extraBody`; navin merges it into Converse `additionalModelRequestFields`:
...
Current scope: navin passes `messages`, `system`, `inferenceConfig`, `toolConfig`, and `additionalModelRequestFields`. Bedrock Prompt Management, Guardrails, `serviceTier`, and other top-level Converse options are not first-class config fields yet.
...
```bash
navin agent -m "Reply with one short sentence."
```
...
GitHub Copilot uses OAuth instead of API keys. Requires a [GitHub account with a plan](https://github.com/features/copilot/plans) configured. No `providers.github_copilot` block is needed in `config.json`; `navin provider login` stores the OAuth session outside config.

For GitHub Enterprise / Copilot for Business, set the endpoint overrides you need before login:
```bash
export NAVIN_GITHUB_COPILOT_CLIENT_ID="your-enterprise-client-id"
export NAVIN_GITHUB_DEVICE_CODE_URL="https://ghe.example/login/device/code"
export NAVIN_GITHUB_ACCESS_TOKEN_URL="https://ghe.example/login/oauth/access_token"
export NAVIN_GITHUB_USER_URL="https://api.ghe.example/user"
export NAVIN_COPILOT_TOKEN_URL="https://api.ghe.example/copilot_internal/v2/token"
export NAVIN_COPILOT_BASE_URL="https://copilot-api.ghe.example"
```

**1. Login:**
```bash
navin provider login github-copilot
```

**2. Set model** (merge into `~/.navin/config.json`):
```json
{
  "modelPresets": {
    "copilot": {
...
navin agent -c ~/.navin-telegram/config.json -m "Hello!"

# One-off workspace override on top of that config
navin agent -c ~/.navin-telegram/config.json -w /tmp/navin-telegram-test -m "Hello!"
```
...
OpenCode Zen and OpenCode Go are available through navin's built-in
...
| Provider | Default API base | Model prefix accepted by navin |
|----------|------------------|-----------------------------------|
...
provider-specific model endpoints, and `chat/completions`. navin's OpenCode
...
LongCat is available through navin's built-in OpenAI-compatible provider flow. The default API base already points to `https://api.longcat.chat/openai/v1`, so you usually only need to set `apiKey`.
...
Ant Ling is available through navin's built-in OpenAI-compatible provider flow. The default API base points to `https://api.ant-ling.com/v1`, so you usually only need to set `apiKey`.
...
Some OpenAI-compatible gateways expose request-body extensions such as vLLM guided decoding or local sampling controls. Put those under `extraBody`; navin merges them into the chat-completions request body after its provider defaults:
...
If a custom OpenAI-compatible endpoint exposes a provider-specific thinking toggle, set `thinkingStyle` so navin can translate `reasoningEffort` into the right request body. Supported styles are `thinking_type` (`{"thinking":{"type":"enabled"}}`), `enable_thinking` (`{"enable_thinking": true}`), and `reasoning_split` (`{"reasoning_split": true}`):
...
**2. Add to config** (partial — merge into `~/.navin/config.json`):
```json
{
  "providers": {
    "ollama": {
...
[Atomic Chat](https://atomic.chat/) is a local-first desktop app that exposes an **OpenAI-compatible** HTTP API (default `http://localhost:1337/v1`). This setup applies when you want to run navin against a model on your own machine instead of a hosted API provider.
...
**3. Add to config** (partial — merge into `~/.navin/config.json`):
...
String entries are preset names, not raw model names. In the example above, `"deep"` means `modelPresets.deep`; navin will not interpret it as a provider model ID. Changing a preset updates both `/model <preset>` switching and any fallback chain that references it.
...
Failover normally runs when the primary provider returns a retryable model/provider error before any answer text has been streamed. Stream-stall timeouts are the recovery exception: if the provider already emitted partial answer text and then stalls, navin closes the current stream segment and retries/fails over in a new segment. Typical fallback cases include timeouts, connection errors, 5xx server errors, 429 rate limits, overloads, and quota/balance exhaustion. It does not run for malformed requests, authentication/permission errors, content filtering/refusals, or context-length/message-format errors.

If fallback candidates use smaller `contextWindowTokens` values, navin builds context using the smallest window in the active chain so every candidate can receive the same prompt.
...
Global settings that apply to all channels. Configure under the `channels` section in `~/.navin/config.json`:
...
When a channel `send()` raises, navin retries at the channel-manager layer. By default, `channels.sendMaxRetries` is `3`, and that count includes the initial send.
...
>
> If a channel is completely unreachable, navin cannot notify the user through that same channel. Watch logs for `Failed to send to {channel} after N attempts` to spot persistent delivery failures.

## Web Tools

navin incorporates basic tools for accessing the web. These include searching via APIs, and fetching arbitrary web pages in Markdown format. They are enabled by default, and can be configured in `~/.navin/config.json` under `tools.web`.
...
navin supports multiple web search providers. Configure in `~/.navin/config.json` under `tools.web.search`.
...
navin by default uses [Jina Reader](https://jina.ai/reader/), a third-party API, to convert arbitrary pages into Markdown format for easy digestion by the LLM, with a local fallback based on [readability-lxml](https://github.com/buriy/python-readability) if the former fails.
...
navin supports [MCP](https://modelcontextprotocol.io/) — connect external tool servers and use them as native agent tools.
...
`enabledTools` accepts either the raw MCP tool name (for example `read_file`) or the wrapped navin tool name (for example `mcp_filesystem_write_file`).
...
> [!TIP]
> For production deployments, set both `"restrictToWorkspace": true` and `"tools.exec.sandbox": "bwrap"` in your config. `restrictToWorkspace` enables navin's application-level workspace guards; `tools.exec.sandbox` provides process-level isolation for shell commands.
...
| `tools.webuiAllowRemotePackageInstall` | `false` | When `false`, the WebUI can install missing optional packages only from a browser opened on the same machine as navin. Set to `true` only when a trusted remote admin is allowed to install Python packages into this navin environment. |
...
**Docker security**: The official Docker image runs as a non-root user (`navin`, UID 1000) with bubblewrap pre-installed. When using `docker-compose.yml`, the container drops all Linux capabilities except `SYS_ADMIN` (required for bwrap's namespace isolation).
...
1. A user sends a DM to the bot on a pairing-capable channel where they aren't yet approved. This includes Telegram, Discord, and channels such as Slack or Mattermost when their DM policy is set to `allowlist`.
...
The gateway can run a protected heartbeat cron job that periodically checks `HEARTBEAT.md` in the active workspace. This is enabled by default when you run `navin gateway`.
...
When a user is idle for longer than a configured threshold, navin **proactively** compresses the older part of the session context into a summary while keeping a recent legal suffix of live messages. This reduces token cost and first-token latency when the user returns — instead of re-processing a long stale context with an expired KV cache, the model receives a compact summary, the most recent live context, and fresh input.
...
By default, navin uses `UTC` for runtime time context. If you want the agent to think in your local time, set `agents.defaults.timezone` to a valid [IANA timezone name](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones):
...
By default, each channel × chat ID combination gets its own session. If you use navin across multiple channels (e.g. Telegram + Discord + CLI) and want them to share the same conversation, enable `unifiedSession`:
...
navin ships with built-in skills, and your workspace can also define custom skills under `skills/`. If you want to hide specific skills from the agent, set `agents.defaults.disabledSkills` to a list of skill directory names:
