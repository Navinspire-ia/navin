...
This page collects contributor-facing notes for extending navin. User-facing setup and runtime options live in [`configuration.md`](./configuration.md).

## Adding an LLM Provider

navin uses the provider registry in `navin/providers/registry.py` as the source of truth for LLM provider metadata. Most OpenAI-compatible providers need only two changes.
...
2. Add a field to `ProvidersConfig` in `navin/config/schema.py`:
...
- `navin/audio/transcription_registry.py` owns provider names, aliases, default models, and adapter loading.
- `navin/providers/transcription.py` owns provider-specific HTTP behavior.
...
2. Add a `ProviderSpec` in `navin/providers/registry.py`.
...
3. Add an adapter class in `navin/providers/transcription.py`.
...
4. Register the adapter in `navin/audio/transcription_registry.py`.

```python
TranscriptionProviderSpec(
    name="my_stt",
    default_model="my-default-stt-model",
    adapter="navin.providers.transcription:MySTTTranscriptionProvider",
    aliases=("mystt",),
)
```
...
- config resolution picks the right provider, model, and credentials
- adapter request/response behavior and retry/error handling
- WebUI settings payload/update behavior for the new provider
