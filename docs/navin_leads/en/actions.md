...
| Action | Delivers |
| --- | --- |
| CRM-ready export | Clean CSV with standard columns (company, domain, contact, role, email pattern, phone, country, source, score, signal, next action) plus the import mapping for HubSpot/Salesforce. |
| Meeting prep | One-page brief on account and attendees, 3 pain hypotheses with evidence, discovery questions, likely objections, ideal next step. |
| Pipeline review | Stage distribution, aging and stuck deals, weighted forecast, win-rate by segment, top 5 highest-impact actions this month. |
| Competitor customers | Displacement target list built from public evidence (case studies, logo walls, reviews): competitor used, evidence URL, switch angle per account. |
