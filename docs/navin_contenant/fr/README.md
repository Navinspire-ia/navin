...
3. Filtrez par groupe si besoin (Projet, Investissement, Marketing, Ventes, RH & Formation, Juridique).
4. Cliquez une carte de modèle. Un **dialogue de préparation** s'ouvre : décrivez ce que vous voulez, sélectionnez la **langue de sortie** (obligatoire, jamais déduite de la langue de l'interface) et choisissez un **template visuel** de la catégorie (PPT, Word, PDF, Excel) ou laissez « Auto ».
5. Validez. Le panneau de chat s'ouvre et la commande `/studio` part avec la spécification du modèle, vos précisions, votre brief et le template attaché.
6. L'agent clarifie ou déduit les détails manquants, conçoit la structure (sections, arc narratif, une idée par slide/page), génère le fichier avec du vrai contenu - jamais de lorem ipsum - applique un thème visuel, et rapporte le chemin du fichier plus un sommaire.
...
| Skills préchargés | `pptx-generator`, `docx-generator`, `pdf-generator`, `spreadsheet-analyst`, `presentation-designer`, `professional-writer`, `document-templates`, `archify` |
| Sortie | Un vrai fichier enregistré dans l'espace de travail (`.pptx`, `.docx`, `.pdf`, `.xlsx`, `.csv`) |
...
| CSV | Données texte | Exports tabulaires UTF-8 avec séparateur local annoncé |

La langue sélectionnée est conservée de bout en bout : textes et métadonnées, polices, dates, nombres et devises. L'arabe et les autres langues RTL utilisent une direction et une mise en page adaptées avec des polices couvrant les glyphes requis.
...
50 modèles répartis en 6 groupes - voir [Modèles](./templates.md) pour chaque carte :
...
- **Juridique** - 25 contrats, du NDA et des partenariats jusqu'au SaaS, DPA/RGPD, travail, pacte d'actionnaires et protocole transactionnel

Les contrats utilisent un droit applicable et un mode de règlement des litiges paramétrables. L'agent conserve le HTML source avec l'export DOCX/PDF, contrôle la cohérence contractuelle et ajoute systématiquement un avertissement de revue par un juriste local qualifié avant signature.
