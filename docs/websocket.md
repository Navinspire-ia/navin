...
Navin can act as a WebSocket server, allowing external clients (web apps, CLIs, scripts) to interact with the agent in real time via persistent connections.
...
async def main():
    async with websockets.connect("ws://127.0.0.1:8765/?client_id=alice") as ws:
        ...
        await ws.send(json.dumps({"content": "Hello navin!"}))
...
1. Client sends `GET {tokenIssuePath}` with `Authorization: Bearer {tokenIssueSecret}` (or `X-Navin-Auth` header).
...
`chat_id` is a *capability*: anyone holding a valid WebSocket auth credential and the chat_id can attach to that conversation and see its output. This is safe for navin's local, single-user model. Multi-tenant deployments should namespace chat_ids per user (or introduce a per-tenant auth gate) — navin does not do this today.
...
- An HTTP file server serving the navin media directory
