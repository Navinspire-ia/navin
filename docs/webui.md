# Navin WebUI: Browser Workbench for Self-Hosted AI Agents

<!-- Meta description: Run navin from a browser WebUI with persistent chat sessions, visible tool activity, workspace controls, Apps, MCP presets, Skills, settings, and Automations. -->

The WebUI is navin's browser workbench for persistent chat sessions, visible
agent activity, workspace controls, Apps, Skills, settings, and Automations in
one place.

The published `navin-ai` wheel already includes the WebUI bundle. You only need
...
`navin webui` creates the config/workspace when needed, checks provider setup,
...
Manage the background gateway with `navin gateway status`, `navin gateway
logs`, `navin gateway restart`, and `navin gateway stop`.
...
Open Apps from the sidebar to manage tools that navin can attach to a chat
turn. The default **Ready** view shows only tools that can be used immediately:

- **Apps** are local command-line adapters that navin runs on your machine.
...
Apps intentionally does not list navin runtime support packages such as
...
included in navin and activate automatically when a file is attached. The
equivalent CLI for optional integrations remains `navin plugins`. See
...
extraction tools without requiring an API key. This does not replace navin's
...
know whether navin already has a focused workflow for a task before you ask it
...
navin keeps the correct target context. When an automation runs, it normally
...
- Local triggers, created with `/trigger <name>`, run when you call a local
  command such as `navin trigger trg_8K4P2Q9X "Review PR #4502"`.
...
because navin would not know where to deliver the scheduled turn. Recreate it
...
message. Use the copied `navin trigger ...` command and replace `"message"`
...
install missing navin support packages, such as adding a channel dependency,
...
trusted to change the Python environment that navin runs in. If you publish
...
1. `navin agent -m "Hello!"` works in the same Python environment.
2. `~/.navin/config.json` does not explicitly set `channels.websocket.enabled` to `false`.
3. `navin gateway` is still running.
