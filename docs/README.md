...
Repository docs follow the current source tree and can be newer than the latest package release. For published release docs, visit [navin.wiki](https://navin.wiki/docs/latest/getting-started/navin-overview).
...
| You are comfortable running commands | [Install and Quick Start](./quick-start.md) | `navin status` is healthy and the WebUI or CLI can get one reply |
...
1. Install navin.
2. Run `navin webui` — it starts the gateway and opens the platform in the browser.
3. Follow the alert at the top of the platform: open **Settings → Providers**, add your API key, and pick a model.
4. Send `Hello!` before configuring anything else.

Most people do not need to edit JSON or use a terminal wizard: everything is configured directly in the platform. Use **Settings** for models and built-in capabilities, **Settings → Channels** for chat apps, and **Apps** for CLI App or MCP integrations.
...
| Connect Telegram, Discord, Slack, Email, or another chat app | [Chat Apps](./chat-apps.md) |
...
| Run navin continuously | [Deployment](./deployment.md) |
| Run separate bots or workspaces | [Multiple Instances](./multiple-instances.md) |
| Call navin from Python | [Python SDK](./python-sdk.md) |
...
Full documentation for the WebUI studio modules, in both languages:

| Module | Route | Docs |
|---|---|---|
| Dev — workbench, 32 slash commands, actions, skills, plugins | `#/dev` | [navin_dev](./navin_dev/README.md) |
| Documents — 25 templates (PPTX/DOCX/PDF/XLSX), `/studio` | `#/content` | [navin_contenant](./navin_contenant/README.md) |
| Marketing — campaigns, ad videos, product images, `/campaign` | `#/marketing` | [navin_marketing](./navin_marketing/README.md) |
| SEO — audits, keywords, optimized content, `/seo` | `#/seo` | [navin_seo](./navin_seo/README.md) |
| Leads & Sales — company/people/job search, signals, outreach, `/leads` | `#/leads` | [navin_leads](./navin_leads/README.md) |
| Team — org design, roles, hiring, OKRs, virtual AI teams, `/team` | `#/team` | [navin_equipe](./navin_equipe/README.md) |
...
Configuration examples are usually snippets to merge into `~/.navin/config.json`, not complete replacement files. The docs use camelCase because navin writes config that way. Keep real API keys, bot tokens, and passwords out of issues and public logs.

## Extend or Contribute

These pages explain implementation and extension points. You do not need them to install or operate navin.

| Goal | Read |
|---|---|
| Understand source ownership and runtime flow | [Architecture](./architecture.md) |
| Set up a development environment | [Development](./development.md) and [AGENTS.md](../AGENTS.md) |
...
If a command or screen no longer matches these docs, please [open an issue](https://github.com/EIAGEN/navin-claw/issues) with your navin version, operating system, and the page that needs correction.
