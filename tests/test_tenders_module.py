...
class TendersAgentWiringTest(unittest.TestCase):
    ...
    def test_shell_opens_tenders_first(self) -> None:
        ...
        self.assertIn("visibleStudioModules(", sidebar)
        modules = (ROOT / "webui/src/lib/studio-modules.ts").read_text(encoding="utf-8")
        default_order = modules.split("STUDIO_MODULE_IDS =", 1)[1].split("] as const", 1)[0]
        self.assertLess(default_order.find('"tenders"'), default_order.find('"trading"'))
        self.assertLess(default_order.find('"tenders"'), default_order.find('"leads"'))
        ...
        self.assertIn('data-testid="tenders-collect"', workspace)
    ...
    def test_desk_is_single_composition_not_a_split(self) -> None:
        ...
        self.assertIn("NoticeFilters", desk)
        self.assertIn("filter={facet}", desk)
        self.assertIn("onChange={setFacet}", desk)
        ...
        self.assertIn("FollowUpButton", desk)
        self.assertIn('data-testid="tenders-follow-up-panel"', desk)
        self.assertIn("ModelRouting", desk)
        self.assertIn("filterEmptyTitle", desk)
