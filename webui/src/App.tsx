...
import { DevWorkbench } from "@/components/dev/DevWorkbench";
import { StudioWorkspace, type StudioModule } from "@/components/studio/StudioWorkspace";
...
import { NavinClient } from "@/lib/navin-client";
...
function readShellRoute(): ShellRoute {
  ...
  if (path === "/leads") {
    return { view: "leads", activeKey, settingsSection: "overview" };
  }
  if (path === "/team") {
    return { view: "team", activeKey, settingsSection: "overview" };
  }
...
function Shell({
  ...
  const [devNewAgentPromptOpen, setDevNewAgentPromptOpen] = useState(false);
  ...
  // Dev view: resizable + collapsible chat pane.
  const [devChatOpen, setDevChatOpen] = useState(true);
  ...
  // Workbench views (dev/content/marketing/seo): register the active chat as an agent tab.
  useEffect(() => {
    if (!isWorkbenchView(view) || !activeKey) return;
    setDevChatTabs((prev) => (prev.includes(activeKey) ? prev : [...prev, activeKey]));
  }, [view, activeKey, sessions]);

  // Drop dev tabs whose sessions were deleted. Keep the active key even when
  // the sessions list has not caught up with a freshly created chat yet.
  useEffect(() => {
    if (loading) return;
    setDevChatTabs((prev) => {
      const next = prev.filter(
        (key) => key === activeKey || sessions.some((s) => s.key === key),
      );
      return next.length === prev.length ? prev : next;
    });
  }, [sessions, loading, activeKey]);

  const onSelectDevTab = useCallback(
    (key: string) => {
      navigate({
  ...
  const onCloseDevTab = useCallback(
    (key: string) => {
      setDevChatTabs((prev) => {
        ...
        if (activeKey === key) {
          navigate({
            view: isWorkbenchView(view) ? view : "dev",
            activeKey: next.length ? next[next.length - 1] : null,
            settingsSection: "overview",
          });
        }
        return next;
  ...
  const devTabTitle = useCallback(
    ...
    [sessions, sidebarState.title_overrides, t],
  );
  ...
  >(null);
  const onOpenFileInDevEditor = useCallback((path: string) => {
  ...
  >(null);
  const onRunDevAction = useCallback((text: string) => {
    setDevChatOpen(true);
  ...
  }, [activeWorkspaceScope, createChat, navigate, t, view]);

  const onConfirmNewDevAgent = useCallback(
    (name: string) => {
      setDevNewAgentPromptOpen(false);
      const cleaned = name.trim();
      void (async () => {
        const chatId = await onCreateChat();
        if (!chatId || !cleaned) return;
        const key = `websocket:${chatId}`;
        void updateSidebarState((current) => ({
          ...current,
          title_overrides: { ...current.title_overrides, [key]: cleaned },
        }));
      })();
    },
    [onCreateChat, updateSidebarState],
  );
  ...
  const onSelectDevProject = useCallback(
    (projectPath: string, projectName?: string) => {
      ...
      if (!trimmed) return;
      const name = projectName || projectNameFromPath(trimmed);
      ...
      if (activeChatId) {
        applyWorkspaceScope(nextScope);
      } else {
        // No active dev session yet: open one bound to the selected project so
        // the file tree, terminal and agent all target the new root.
        void onCreateChat(nextScope);
      }
      void updateSidebarState((current) => {
        ...
        return {
          ...current,
          recent_projects: [{ path: trimmed, name }, ...rest].slice(0, 20),
        };
      });
    },
    [
  ...
  const onOpenTeamStudio = useCallback(() => {
    setSessionSearchOpen(false);
    navigate({ view: "team", activeKey, settingsSection: "overview" });
    setMobileSidebarOpen(false);
  }, [activeKey, navigate]);
  ...
  useEffect(() => {
    ...
    if (isWorkbenchView(view)) {
      const workbenchTitles: Partial<Record<ShellView, string>> = {
        ...
        team: t("sidebar.teamStudio", { defaultValue: "Team" }),
      };
      document.title = t("app.documentTitle.chat", {
    ...
    <ThemeProvider theme={theme}>
      ...
      >
        ...
        >
            ...
            {providerSetupNeeded && !providerBannerDismissed && view === "chat" ? (
              <div className="pointer-events-none absolute inset-x-0 top-0 z-40 flex justify-center px-4 pt-3">
                <div
                  role="alert"
                  className="pointer-events-auto flex max-w-xl items-center gap-3 rounded-xl border border-border bg-background/95 px-4 py-2.5 shadow-lg backdrop-blur"
                >
                  <TriangleAlert className="h-4 w-4 shrink-0 text-foreground" aria-hidden />
                  <p className="min-w-0 text-[13px] leading-snug text-foreground">
                    {t("providerSetup.message", {
                      defaultValue:
                        "No model provider is configured yet. Add one to start chatting.",
                    })}
                  </p>
                  <button
                    type="button"
                    onClick={onOpenModelSettings}
                    className="shrink-0 rounded-lg border border-border bg-foreground px-3 py-1.5 text-[12px] font-medium text-background transition-opacity hover:opacity-85"
                  >
                    {t("providerSetup.action", { defaultValue: "Configure a provider" })}
                  </button>
                  <button
                    type="button"
                    onClick={() => setProviderBannerDismissed(true)}
                    className="shrink-0 rounded-md p-1 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                    aria-label={t("providerSetup.dismiss", { defaultValue: "Dismiss" })}
                  >
            ...
            {isWorkbenchView(view) && (
              <div
                ...
                style={{ right: devChatOpen ? devChatWidth : 0 }}
              >
                {view === "dev" ? (
                  <DevWorkbench
                    ...
                    chatOpen={devChatOpen}
                    onToggleChat={() => setDevChatOpen((open) => !open)}
                    onRunAction={onRunDevAction}
                  />
                ) : (
                  <StudioWorkspace
                    module={view as StudioModule}
                    chatOpen={devChatOpen}
                    onToggleChat={() => setDevChatOpen((open) => !open)}
            ...
            {isWorkbenchView(view) && devChatOpen && (
              <div
                role="separator"
            ...
            <div
              className={cn(
                ...
                isWorkbenchView(view)
                  ? "inset-y-0 right-0 overflow-hidden border-l border-border/55"
                  : "inset-0",
                view !== "chat" && !isWorkbenchView(view) && "invisible pointer-events-none",
                isWorkbenchView(view) &&
                  !devChatOpen &&
                  "invisible pointer-events-none border-l-0",
              )}
              style={
                isWorkbenchView(view) ? { width: devChatOpen ? devChatWidth : 0 } : undefined
              }
            >
              {isWorkbenchView(view) && (
                <div className="flex shrink-0 items-center gap-1 border-b border-border/55 bg-muted/15 px-2 py-1.5">
                  <div className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto">
                    {devChatTabs.map((key) => {
                      const chatId = key.startsWith("websocket:")
                        ? key.slice("websocket:".length)
                        : key;
                      const running = runningChatIds.has(chatId);
                      return (
                        <div
                          key={key}
                          className={cn(
                          ...
                          onClick={() => onSelectDevTab(key)}
                          onDoubleClick={() => onRequestRename(key, devTabTitle(key))}
                          title={t("dev.renameAgentHint", {
                        ...
                        >
                          <Bot
                            ...
                            aria-hidden
                          />
                          <span className="max-w-[9rem] truncate">{devTabTitle(key)}</span>
                          {running ? (
                            <span
                              className="h-1.5 w-1.5 shrink-0 animate-pulse rounded-full bg-foreground/70"
                              aria-label={t("dev.agentRunning", { defaultValue: "Agent running" })}
                            />
                          ) : null}
                          <button
                            type="button"
                            ...
                            aria-label={t("dev.closeAgentTab", { defaultValue: "Close agent tab" })}
                          >
                  ...
                  <button
                    type="button"
                    onClick={() => setDevNewAgentPromptOpen(true)}
                    className="shrink-0 rounded-md p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground"
                    aria-label={t("dev.newAgent", { defaultValue: "New agent" })}
                    title={t("dev.newAgent", { defaultValue: "New agent" })}
                  >
              ...
              <ThreadShell
                ...
                skills={skills}
                onOpenFileInEditor={view === "dev" ? onOpenFileInDevEditor : undefined}
                autoSendRequest={isWorkbenchView(view) ? devAutoSendRequest : null}
              />
              </div>
            </div>
        ...
        />
        <RenameChatDialog
          open={devNewAgentPromptOpen}
          title=""
          dialogTitle={t("dev.nameAgentTitle", { defaultValue: "Name this session" })}
          description={t("dev.nameAgentDescription", {
            defaultValue:
              "Give the agent session a name that describes its task (e.g. “API refactor”, “Landing page”).",
          })}
          placeholder={t("dev.nameAgentPlaceholder", { defaultValue: "Session name" })}
          onCancel={() => setDevNewAgentPromptOpen(false)}
          onConfirm={onConfirmNewDevAgent}
        />
        <RenameChatDialog
