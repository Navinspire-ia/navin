...
import { ideNavigationHash } from "@/lib/external-url";
...
function candidatePreviewPaths(path: string): string[] {
  const raw = path.trim().replace(/\\/g, "/");
  if (!raw) return [];
  if (raw.includes("/")) return [raw];
  const extras = [`scrape/${raw}`, `exports/${raw}`, `output/${raw}`, `out/${raw}`];
  if (/^test_.*\.py$/i.test(raw)) extras.unshift(`tests/${raw}`, `test/${raw}`);
  if (/\.(tsx?|jsx?)$/i.test(raw)) extras.unshift(`src/${raw}`, `webui/src/${raw}`);
  return [raw, ...extras];
}
...
function InferredFileReferenceChip({
  ...
  useEffect(() => {
    ...
    if (!resolve) return;
    let cancelled = false;
    const candidates = candidatePreviewPaths(path);
    void (async () => {
      for (const candidate of candidates) {
        try {
          const available = await resolve(candidate);
          if (cancelled) return;
          if (available) {
...
function normalizeSafeDetails(children: MarkdownAstNode[]): MarkdownAstNode[] {
  ...
  for (let index = 0; index < children.length; index += 1) {
    ...
    next.push({
      type: "navinSafeHtmlDetails",
      data: { hName: "details" },
      children: [
        {
          type: "navinSafeHtmlSummary",
          data: { hName: "summary" },
...
export default function MarkdownTextRenderer({
  ...
  const components = useMemo<Components>(
    () => ({
      code({ className: cls, children: kids, node: _node, ...props }) {
        ...
        // Inline chip: sized and tinted by the `.markdown-content` rules in
        // globals.css so the whole answer shares one typographic system.
        return (
      ...
      a({ href, children: markdownChildren, node: _node, ...props }) {
        ...
        const ideHash = ideNavigationHash(href || "");
        if (ideHash) {
          return (
            <a
              {...props}
  ...
  return (
    <div
      className={cn(
        // Reading rhythm (sizes, weights, spacing, colors) lives in the
        // `.markdown-content` rules of globals.css: one system for every
        // answer, scaled by the Density preference. Only what Typography
        // must not do stays here.
        "markdown-content prose max-w-none dark:prose-invert",
        "prose-a:text-blue-500 hover:prose-a:text-blue-600 dark:prose-a:text-blue-300 dark:hover:prose-a:text-blue-200",
