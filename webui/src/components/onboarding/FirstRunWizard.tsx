...
export function FirstRunWizard({
  ...
  return (
    <div
      className={cn(
        "host-no-drag fixed inset-0 z-[80] overflow-y-auto overscroll-contain bg-background/80 p-3 backdrop-blur-sm sm:p-4",
        className,
      )}
    ...
    >
      <div className="flex min-h-full items-center justify-center py-2">
        <div className="flex h-[min(92dvh,calc(100dvh-1.5rem))] w-full max-w-lg min-h-0 flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-xl">
          ...
          <div
            data-testid="first-run-wizard-scroll"
            className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-6"
          >
            {step === 1 ? (
              <div className="mt-6 grid gap-2 pb-2">
                {LANGS.map((lang) => (
                  <button
                    key={lang.code}
                    type="button"
