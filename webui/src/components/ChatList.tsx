...
function SessionActivityIndicator({
  ...
  if (state === "running") {
    ...
    return (
      ...
      >
        <span className="h-3 w-3 animate-spin rounded-full border border-primary/25 border-t-primary [animation-duration:1.4s] motion-reduce:animate-none" />
      </span>
    );
  }
  ...
  if (state === "updated") {
    ...
    return (
      <span
        ...
        className="grid h-4 w-4 shrink-0 place-items-center"
      >
        <span className="h-2 w-2 rounded-full bg-foreground/60 shadow-[0_0_0_2px_hsl(var(--foreground)/0.12)]" />
      </span>
    );
  }
