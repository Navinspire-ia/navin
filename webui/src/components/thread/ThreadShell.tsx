...
interface ThreadShellProps {
  ...
  autoSendRequest?: { text: string; nonce: number } | null;
}
...
export function ThreadShell({
  ...
  const {
    ...
    streamError,
    dismissStreamError,
  } = useNavinStream(chatId, initial, hasPendingToolCalls, handleTurnEnd);
  ...
  useEffect(() => {
    ...
    if (chatId) {
      handleThreadSend(autoSendRequest.text);
    } else {
      void handleWelcomeSend(autoSendRequest.text);
    }
  }, [autoSendRequest, booting, chatId, handleThreadSend, handleWelcomeSend, isStreaming]);

  const handleOpenFilePreview = useCallback((path: string) => {
    // In the Dev view, file links from chat steps open in the code editor
    // (Cursor-style) instead of the side preview panel.
