...
const QUEUED_PROMPTS_STORAGE_PREFIX = "navin.webui.composerQueuedGuidance.v1:";
...
type MentionCandidate =
  ...
  | { kind: "team"; name: string; member: TeamMember };
...
export function ThreadComposer({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void fetchTeamRoster(clientToken)
      .then((payload) => {
        if (!cancelled) setTeamMembers(payload.members);
      })
      .catch(() => {
        /* roster is optional — mentions simply stay app/preset only */
      });
    return () => {
      cancelled = true;
    };
  ...
  const filteredMentionCandidates = useMemo<MentionCandidate[]>(() => {
    ...
    const teamCandidates: MentionCandidate[] = teamMembers
      .filter((member) => {
        const haystack = [member.name, member.role, member.specialty]
          .join(" ")
          .toLowerCase();
        return haystack.includes(cliAppMention.query);
      })
      .map((member) => ({ kind: "team", name: member.name, member }));
    return [...teamCandidates, ...cliCandidates, ...mcpCandidates].slice(0, 8);
  }, [cliAppMention, cliApps, mcpPresets, teamMembers]);
  ...
  const chooseMentionCandidate = useCallback(
    (candidate: MentionCandidate) => {
      ...
      if (candidate.kind === "team") {
        // Route the message to the member's workflow command with their
        // role and skills as the persona header; the user's ask follows.
        ...
        const persona =
          ...
          + `${specialty}${skills} Stay in this role.\n\n`;
        const rest = `${value.slice(0, cliAppMention.start)}${suffix}`.trim();
        next = `${persona}${rest}`;
        nextCursor = next.length;
      } else {
        const mention = `@${candidate.name}${suffix.startsWith(" ") ? "" : " "}`;
        next = `${value.slice(0, cliAppMention.start)}${mention}${suffix}`;
        nextCursor = cliAppMention.start + mention.length;
      }
      setValue(next);
      setCursorPosition(nextCursor);
      setCliAppMenuDismissed(true);
        ...
        {candidates.map((candidate, index) => {
          ...
          const ariaDescription = candidate.kind === "cli"
            ...
            : candidate.kind === "mcp"
              ...
              : t("thread.composer.mentions.teamDescription", {
                  name,
                  defaultValue: `Call ${name} from your team`,
                });
          return (
            <button
...
function MentionCandidateLogo({
  ...
  if (candidate.kind === "team") {
    return (
      <span
        className={cn(
          "flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-border/60 text-[12px]",
