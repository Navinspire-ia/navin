...
function FileEditRow({
  ...
  // Raw tool errors never reach the client thread: a failed step is part of
  // the agent's normal self-correction loop. Details stay in the gateway logs.
  ...
  return (
    <ActivityStep
      ...
      marker={(
        <span
          className={cn(
            ...
            failed && "border-muted-foreground/28 text-muted-foreground/60",
      ...
      title={edit.absolute_path || edit.path}
    ...
    >
      {failed ? (
        <span className="block max-w-[42rem] truncate text-[12px] leading-4 text-muted-foreground/60">
          {failureDetail}
        </span>
      ) : null}
      {showDiff ? (
...
function FileUnifiedDiff({
  ...
  const renderBody = () => (
    ...
    >
      ...
      {visibleDiff.hiddenLineCount > 0 ? (
        <div className="border-t border-border/45 bg-muted/30 px-2 py-1">
          <button
            type="button"
            className={cn(
              "inline-flex items-center gap-1 rounded px-1 py-0.5 text-[10.5px] font-medium",
      ...
      {diff.truncated ? (
        <div
          className="flex flex-wrap items-center gap-x-2 gap-y-1 border-t border-border/45 bg-muted/35 px-2 py-1 text-[10.5px] text-muted-foreground"
    ...
    <div className="mt-1">
      <button
        ...
        className={cn(
          ...
          "text-[10.5px] font-medium text-muted-foreground transition-colors hover:bg-muted/50",
        )}
      >
        <ChevronRight
...
function DiffHunkGap({ lineCount }: { lineCount: number }) {
  ...
  return (
    <div
      className="flex items-center gap-2 bg-muted/35 px-2 py-1 text-[10.5px] text-muted-foreground"
