...
export function channelSetup(feature: NavinFeatureInfo): ChannelSetupPresentation {
  return CHANNEL_PRESENTATION[feature.name]?.setup ?? {
    summary:
      "Enable turns on this channel in navin, but this integration still needs platform-specific setup before it can receive messages.",
    steps: [
      `Open the ${feature.display_name} panel and fill the fields on the right.`,
      "Save and enable the channel, then send a test message from that app.",
    ],
  };
}
...
export function channelDisplayName(feature: NavinFeatureInfo): string {
  return CHANNEL_PRESENTATION[feature.name]?.displayName ?? feature.display_name;
}

export function channelDescription(feature: NavinFeatureInfo, t: ReturnType<typeof useTranslation>["t"]): string {
  const fallback =
    CHANNEL_PRESENTATION[feature.name]?.description ??
    `Use navin from ${channelDisplayName(feature)}.`;
  return t(`settings.channels.items.${feature.name}.description`, { defaultValue: fallback });
}

export function channelRequirements(feature: NavinFeatureInfo, t: ReturnType<typeof useTranslation>["t"]): string {
  const fallback =
...
export function channelMatchesFilter(feature: NavinFeatureInfo, filter: ChannelFilter): boolean {
...
export function channelSearchText(feature: NavinFeatureInfo): string {
  return [
