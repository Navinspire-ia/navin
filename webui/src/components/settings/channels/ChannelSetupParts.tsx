...
import { channelDisplayName } from "@/components/settings/channels/ChannelIdentity";
...
export function ChannelGuideLink({
  ...
  const steps = [...(setup.steps || []), ...(setup.extraGuide || [])];
  if (!steps.length && !setup.tryIt) return null;
  const label = t(`settings.channels.items.${feature.name}.setup.docsLabel`, {
    defaultValue: setup.docsLabel ?? tx("settings.channels.setupGuide", "Open setup"),
  });
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[min(80vh,36rem)] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {channelDisplayName(feature)} {tx("settings.channels.setupTitle", "setup")}
            </DialogTitle>
            <DialogDescription>
              {t(`settings.channels.items.${feature.name}.setup.summary`, {
                defaultValue: setup.summary ?? "",
              })}
            </DialogDescription>
          </DialogHeader>
          <ol className="space-y-2 text-[13px] leading-5 text-foreground">
            {steps.map((step, index) => (
              <li key={`${index}-${step.slice(0, 24)}`} className="flex gap-2">
                ...
                <span>
                  {t(`settings.channels.items.${feature.name}.setup.guide.${index}`, {
                    defaultValue: step,
                  })}
                </span>
              </li>
          ...
          {setup.tryIt ? (
            <p className="rounded-[12px] border border-border/55 bg-muted/40 px-3 py-2 text-[12.5px] text-muted-foreground">
              <span className="font-medium text-foreground">{tx("settings.channels.tryIt", "Try it")} </span>
              {t(`settings.channels.items.${feature.name}.setup.tryIt`, { defaultValue: setup.tryIt })}
            </p>
          ) : null}
        </DialogContent>
      </Dialog>
...
export function ChannelSetupLinks({
  ...
  return (
    <div className="mt-3 flex flex-wrap items-center gap-2">
      <ChannelOfficialLink feature={feature} setup={setup} token={token} />
      <ChannelGuideLink feature={feature} setup={setup} docsBaseUrl={docsBaseUrl} compact />
    </div>
  );
}
...
export function ChannelOfficialLink({
  ...
  return (
    <button
      type="button"
      onClick={() => {
        void openExternalUrl(token, setup.officialUrl!);
      }}
...
export function ChannelValidationChecks({
  validation,
  token,
}: {
            ...
            <div className="min-w-0 flex-1">
              ...
              {check.action_url ? (
                <button
                  type="button"
                  onClick={() => {
                    void openExternalUrl(token, check.action_url!);
                  }}
