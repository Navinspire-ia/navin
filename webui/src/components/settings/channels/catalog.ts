...
const NAVIN_DOCS_URL = "https://navin.wiki/docs/latest";
const CHAT_APPS_DOCS_URL = `${NAVIN_DOCS_URL}/getting-started/chat-apps`;
...
export const CHANNEL_PRESENTATION: Record<string, ChannelPresentation> = {
  websocket: {
    ...
    setup: {
      ...
      summary: "WebSocket is required by the browser workbench and is prepared by the navin webui command.",
      steps: [
        "Start the workbench with navin webui so the local gateway and WebSocket channel are enabled together.",
  ...
  whatsapp: {
    ...
    setup: {
      ...
      command: "navin channels login whatsapp",
  ...
  signal: {
    ...
    setup: {
      ...
      summary:
        "Signal connects through a local signal-cli HTTP daemon. Run the daemon first, then point navin at it.",
      steps: [
  ...
  msteams: {
    ...
    setup: {
      ...
      steps: [
        ...
        "Set the bot messaging endpoint to the navin Teams callback path.",
