...
export function SettingsView({
  ...
  useEffect(() => {
    if (!["tools", "models", "providers", "browser", "runtime"].includes(activeSection)) return;
    let cancelled = false;
  ...
  const handleApiServiceAction = async (
    ...
    try {
      ...
      setApiService(payload);
      const refreshed = await fetchNavinFeatures(token);
      setNavinFeatures(refreshed);
      ...
      case "runtime":
        return (
          <RuntimeSettings
            ...
            capabilitiesLoading={navinFeaturesLoading}
            capabilityAction={navinFeatureAction}
            capabilityError={navinFeaturesError}
      ...
      >
        <div
          className={cn(
            ...
            activeSection === "tools" ? "max-w-[1240px] xl:px-10" : "max-w-[920px]",
        ...
        {SETTINGS_NAV_ITEMS.map(({ key, icon: Icon, fallback }) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative flex h-9 w-auto shrink-0 snap-start items-center gap-2 rounded-full px-3 text-left text-[13px] font-medium transition-colors lg:w-full lg:rounded-xl lg:px-2.5",
                active
                  ? "bg-primary/10 text-primary lg:shadow-[inset_3px_0_0_0_hsl(var(--primary))]"
...
function OverviewSettings({
  ...
  const videoStatus = videoGeneration.enabled
    ? tx("settings.values.enabled", "Enabled")
    : tx("settings.values.disabled", "Disabled");
  const videoCaption = `${providerDisplayLabel(videoGeneration.providers, videoGeneration.provider)} · ${
      ...
      {configuredProviders.length === 0 ? (
        <div className="rounded-2xl border border-primary/20 bg-primary/[0.06] px-4 py-4 sm:px-5">
          <p className="text-[14px] font-medium text-foreground">
            {tx("settings.models.needsProviderTitle", "Configure a provider first")}
          </p>
          <p className="mt-1 text-[13px] leading-5 text-muted-foreground">
            {tx(
              "settings.models.needsProviderHelp",
              "Add an API key or local endpoint under Providers, then pick the model here.",
            )}
          </p>
          <Button size="sm" onClick={onOpenProviders} className="mt-3 rounded-full">
...
function ProvidersSettings({
  ...
  const configuredProviders = orderUnconfiguredProviders(
    settings.providers.filter(
      (provider) => provider.configured && !RETIRED_LLM_PROVIDERS.has(provider.name),
    ),
  );
      ...
      <section>
        ...
        <SettingsGroup>
          <SettingsRow
            ...
            description={tx("settings.help.videoGeneration", "Expose generate_video in chats when a configured video provider is available.")}
          >
            <ToggleButton
      ...
      <section>
        ...
        <SettingsGroup>
          <SettingsRow
            ...
            description={tx("settings.help.videoModel", "Model name sent to the selected video provider, e.g. veo-3.1-generate-preview, sora-2, MiniMax-Hailuo-02.")}
          >
            <Input
              value={form.model}
...
function ToolsSettings({
  ...
  const filterOptions: Array<{ value: ChannelFilter; label: string; count: number }> = [
    { value: "all", label: tx("settings.tools.filterAll", "All"), count: allChannels.length },
    { value: "on", label: tx("settings.tools.filterOn", "Connected"), count: enabledCount },
    { value: "off", label: tx("settings.tools.filterOff", "Available"), count: offCount },
  ];
        ...
        <section className="shrink-0 space-y-5">
          <div className="rounded-2xl border border-border/50 bg-gradient-to-br from-primary/[0.07] via-card to-teal/10 px-4 py-5 sm:px-5">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                <h3 className="text-[16px] font-semibold tracking-[-0.02em] text-foreground">
                  {tx("settings.tools.heroTitle", "Tools")}
                </h3>
                <p className="mt-1 max-w-[42rem] text-[13px] leading-6 text-muted-foreground">
                  {tx(
                    "settings.tools.description",
                    "Connect chat apps, email, and the local WebUI so Navin can talk to your teams.",
                  )}
                </p>
                <div className="mt-3 flex flex-wrap gap-2 text-[12px]">
                  <span className="rounded-full bg-background/80 px-2.5 py-1 font-medium text-foreground ring-1 ring-border/60">
                    {t("settings.tools.caption", {
                      enabled: enabledCount,
                      total: allChannels.length,
                      defaultValue: "{{enabled}} connected · {{total}} tools",
                    })}
                  </span>
                </div>
              </div>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
            <div className="relative min-w-0 flex-1">
              ...
              <Input
                value={query}
                onChange={(event) => onQueryChange(event.target.value)}
                placeholder={tx("settings.tools.searchPlaceholder", "Search tools")}
                className="h-11 rounded-2xl border-border/60 bg-card/90 pl-10 text-[13px] shadow-sm"
              />
            </div>
      ...
      >
        {loading && !navinFeatures ? (
          ...
          <div className="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_minmax(400px,460px)] gap-6 overflow-hidden">
            ...
            <div className="min-h-0 overflow-y-auto overscroll-contain rounded-2xl border border-border/45 bg-card/70 p-4 pr-1 shadow-sm">
              {setupPanel}
            </div>
          </div>
        ) : showingCompactDetail ? (
          <div className="pb-6">
            ...
            >
              ...
              {tx("settings.tools.backToTools", "All tools")}
            </button>
            {setupPanel}
          </div>
        ) : (
...
function AppsCatalogSettings({
  ...
  return (
    <div className="space-y-5">
      ...
      {requiresRestartPending ? (
        <RestartRequiredNotice
          message={tx("settings.apps.restartRequired", "Restart navin to apply updated apps and features.")}
          onRestart={onRestart}
          isRestarting={isRestarting}
        />
      ) : null}
...
function RuntimeSettings({
  ...
  langfuseFeature?: NavinFeatureInfo;
  ...
  return (
    <div className="space-y-7">
      <section>
        ...
        <SettingsGroup>
          <SettingsRow title={tx("settings.rows.botName", "Bot name")} description={tx("settings.help.botName", "Shown wherever navin uses a display name.")}>
            <Input
              value={form.botName}
          ...
          <SettingsRow
            ...
            description={
              settings.observability?.configured
                ? tx("settings.observability.configured", "Tracing credentials are available to navin.")
                : tx(
                    "settings.observability.environment",
                    "Set LANGFUSE_SECRET_KEY and LANGFUSE_PUBLIC_KEY, then restart navin.",
                  )
            }
          >
            {capabilitiesLoading ? (
