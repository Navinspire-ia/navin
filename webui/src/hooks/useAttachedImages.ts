...
function mimeForFile(file: File): string {
  ...
  if (ACCEPTED_VIDEO_MIMES.has(file.type)) return file.type;
  const byVideoName = VIDEO_MIME_BY_EXTENSION.get(extensionOf(file.name));
  if (byVideoName) return byVideoName;
  if (ACCEPTED_AUDIO_MIMES.has(file.type)) return file.type;
  const byAudioName = AUDIO_MIME_BY_EXTENSION.get(extensionOf(file.name));
  if (byAudioName) return byAudioName;
...
export function isVideoAttachment(file: File): boolean {
  return ACCEPTED_VIDEO_MIMES.has(mimeForFile(file));
}

export function isAudioAttachment(file: File): boolean {
...
export function acceptedAttachmentKind(file: File): AttachmentKind | null {
  ...
  // Videos ride the raw-bytes path, not the image normalizer: re-encoding a
  // video in a canvas worker is not possible, and the gateway wants the
  // original container to sample frames from. Audio likewise: the gateway
  // transcribes the original file.
  if (ACCEPTED_VIDEO_MIMES.has(mime)) return "file";
  if (ACCEPTED_AUDIO_MIMES.has(mime)) return "file";
