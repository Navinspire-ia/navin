...
function recentProjects(value: unknown): Array<{ path: string; name: string }> {
  if (!Array.isArray(value)) return [];
  const out: Array<{ path: string; name: string }> = [];
  const seen = new Set<string>();
  for (const item of value) {
    if (!item || typeof item !== "object") continue;
    const entry = item as { path?: unknown; name?: unknown };
...
export function normalizeSidebarState(raw: unknown): SidebarStatePayload {
  ...
  return {
    ...
    chat_order: uniqueStrings(value.chat_order),
    ...
    module_by_key: stringMap(
      (value as { module_by_key?: unknown }).module_by_key,
    ),
    module_by_path: stringMap(
      (value as { module_by_path?: unknown }).module_by_path,
    ),
    collapsed_groups: boolMap(value.collapsed_groups),
    recent_projects: recentProjects(value.recent_projects),
    view: {
      density,
      show_previews: Boolean(view.show_previews),
      // Always ON: no settings toggle yet, and older persists may have
      // falsely stored ``false`` when the field was missing.
      show_timestamps: true,
      show_archived: Boolean(view.show_archived),
      sort,
      theme: view.theme === "light" ? "light" : "dark",
    },
...
function pruneMissingSessions(
  ...
  return {
    ...
    chat_order: filterKeys(state.chat_order),
    title_overrides: filterMap(state.title_overrides),
    project_name_overrides: filterMap(state.project_name_overrides),
    tags_by_key: filterMap(state.tags_by_key),
    module_by_key: filterMap(state.module_by_key),
  };
}
