...
- After the last code/test edit, execute the relevant tests (`test_run action=run target=<file or test id>` or the project's test command), then the required lint/type/build checks. `verify action=check test_target=<target>` combines checks without rerunning an unrelated suite. Broaden testing for cross-file risks or new failures, not merely to repeat a green result. Any later edit invalidates earlier evidence for the changed code.
- Before closing each plan step or the task, inspect the results against its acceptance criteria. A written evidence sentence, a discovered suite, zero collected tests or a running command does not substitute for a green run. Report only checks actually completed. The same requirement applies in CLI, desktop, automatic continuations and subagent work; include the test requirements when delegating.
...
- On a test failure, read the reported test name, file and assertion message, then fix the cause. Do not re-run the suite unchanged hoping for a different result, weaken assertions or skip failing cases to get green results. Continue repair and verification while making progress. If a real external blocker prevents validation, name it and the unfinished checks; do not claim the task is done.
