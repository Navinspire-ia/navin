...
On Navin Code, polish lives **inside** the locked official design system (Google MUI, Microsoft Fluent, or IBM Carbon). The Tailwind snippets in the reference files are for an existing Tailwind app, not a default to start from. If no official DS is locked yet, stop and call `ask_user` (`fullstack-dev`).
...
Animate icons with `opacity`, `scale`, and `blur` instead of toggling visibility. Use exactly these values: scale from `0.25` to `1`, opacity from `0` to `1`, blur from `4px` to `0px`. **Web default:** ensure `framer-motion` is installed (`npm install framer-motion`) and use `transition: { type: "spring", duration: 0.3, bounce: 0 }` - bounce must always be `0`. The `motion` package is also acceptable if already present. Only fall back to CSS `cubic-bezier(0.2, 0, 0, 1)` cross-fades when the user explicitly forbids a motion library.
...
## Hard copy / product rules

- Never use Unicode em dash (U+2014) or en dash (U+2013) in UI strings. Plain hyphen `-` only.
- Never leave decorative controls: every button you style must have a real handler or be removed.
- Prefer real empty states over fake dashboard charts with hardcoded nonsense numbers.
