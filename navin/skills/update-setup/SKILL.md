---
name: update-setup
description: One-time setup wizard for the navin upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `navin --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v navin || true
python -m pip show navin-ai || true
pipx list | sed -n '/navin-ai/,+3p' || true
uv tool list | sed -n '/navin-ai/,+3p' || true
```
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "navin-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "navin-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "navin-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade navin to the latest version. Triggers: upgrade navin, update navin, 升级navin, 更新navin."
---
...
4. Use `exec` to verify: `navin --version`
5. Tell the user the new version. Say: "Run `/restart` to restart navin and apply the update. If `/restart` is unavailable in this channel, restart the navin process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade navin' when you want to update."
