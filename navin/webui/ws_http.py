...
from navin.webui.team_roster import read_team_roster, write_team_roster
from navin.webui.thread_disk import delete_webui_thread
from navin.webui.transcript import build_webui_thread_response
...
class GatewayHTTPHandler:
    ...
    async def _dispatch_session_routes(self, request: WsRequest, got: str) -> Response | None:
        ...
        if m:
            return self._handle_file_tree(request, m.group(1))
    ...
    def _handle_file_preview(self, request: WsRequest, key: str) -> Response:
        ...
        try:
            ...
            if is_probe:
                ...
                payload = file_preview_payload(path, scope=scope, allow_any=allow_any)
    ...
    def _handle_file_tree(self, request: WsRequest, key: str) -> Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        decoded_key = _decode_api_key(key)
        if decoded_key is None:
        ...
        try:
            scope = self.workspaces.scope_for_session_key(decoded_key)
            payload = file_tree_payload(path, scope=scope)
    ...
    async def _handle_metagraph(self, request: WsRequest, key: str) -> Response:
        ...
        try:
            scope = self.workspaces.scope_for_session_key(decoded_key)
            payload = await asyncio.to_thread(metagraph_payload, scope)
    ...
    async def _handle_project_search(self, request: WsRequest, key: str) -> Response:
        ...
        try:
            ...
            payload = await asyncio.to_thread(
                search_payload, scope, term, regex=regex, case_sensitive=case_sensitive,
            )
    ...
    async def _handle_git_changes(self, request: WsRequest, key: str) -> Response:
        ...
        try:
            scope = self.workspaces.scope_for_session_key(decoded_key)
            payload = await asyncio.to_thread(git_changes_payload, scope)
        except ProjectSearchError as e:
            return _http_error(e.status, e.message)
        return _http_json_response(payload)
    ...
    async def _handle_git_diff(self, request: WsRequest, key: str) -> Response:
        ...
        try:
            scope = self.workspaces.scope_for_session_key(decoded_key)
            payload = await asyncio.to_thread(git_diff_payload, scope, file_arg)
        except ProjectSearchError as e:
            return _http_error(e.status, e.message)
        return _http_json_response(payload)
    ...
    async def _dispatch_misc_routes(
        ...
        if got == "/api/webui/fs/list":
            return self._handle_webui_fs_list(request)
        if got == "/api/webui/git/status":
            return self._handle_webui_git_status(request)
        if got == "/api/webui/diagnostics":
            return self._handle_webui_diagnostics(request)
        ...
        if got == "/api/webui/team-roster":
            return self._handle_webui_team_roster(request)
        if got == "/api/webui/team-roster/update":
            return self._handle_webui_team_roster_update(request)
    ...
    def _handle_webui_fs_list(self, request: WsRequest) -> Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        ...
        try:
            payload = fs_list_payload(raw_path, show_hidden=show_hidden)
        except FsBrowseError as exc:
            return _http_error(exc.status, exc.message)
        return _http_json_response(payload)

    def _handle_webui_git_status(self, request: WsRequest) -> Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        ...
        raw_path = _query_first(query, "path") or ""
        try:
            payload = git_status_payload(raw_path)
        except ProjectInsightsError as exc:
            return _http_error(exc.status, exc.message)
        return _http_json_response(payload)

    def _handle_webui_diagnostics(self, request: WsRequest) -> Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        ...
        raw_path = _query_first(query, "path") or ""
        try:
            payload = file_diagnostics_payload(raw_path)
        except ProjectInsightsError as exc:
            return _http_error(exc.status, exc.message)
        return _http_json_response(payload)
    ...
    async def _handle_webui_plugins_install(self, request: WsRequest) -> Response:
        ...
        try:
            payload = install_plugin(source=source, location=location, name=name)
    ...
    def _handle_webui_team_roster(self, request: WsRequest) -> Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        return _http_json_response(read_team_roster())

    def _handle_webui_team_roster_update(self, request: WsRequest) -> Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        query = _parse_query(request.path)
        raw_roster = _query_first(query, "roster")
        if raw_roster is None:
            return _http_error(400, "missing roster")
        try:
            decoded = json.loads(raw_roster)
        except json.JSONDecodeError:
            return _http_error(400, "roster must be JSON")
        if not isinstance(decoded, dict):
            return _http_error(400, "roster must be an object")
        try:
            roster = write_team_roster(decoded)
        except ValueError as e:
            return _http_error(400, str(e))
        except OSError:
            self._log.exception("failed to write team roster")
            return _http_error(500, "failed to write team roster")
        return _http_json_response(roster)
