...
_TOOL_RESULTS_DIR = ".navin/tool-results"
...
def load_bundled_template(template_name: str) -> str | None:
    """Read a bundled template file from the navin package."""
    from importlib.resources import files as pkg_files

    with suppress(Exception):
        tpl = pkg_files("navin") / "templates" / template_name
        if tpl.is_file():
