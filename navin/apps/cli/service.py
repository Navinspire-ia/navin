...
# Community extension registry (upstream project name; the repo is external).
NAVIN_EXTENSION_REGISTRY_URL = "https://raw.githubusercontent.com/Re-bin/nanobot-extension/main/registry.json"
NAVIN_EXTENSION_RAW_BASE = "https://raw.githubusercontent.com/Re-bin/nanobot-extension/main"
_CATALOG_SOURCES = (
...
class CliAppManager:
    ...
    def catalog(
        ...
        *,
        ...
        apps = filter_cli_catalog(list(apps_by_name.values()))
        return apps, max(updated_values) if updated_values else None
    ...
    def _manifest_source(self, app: dict[str, Any]) -> str:
        ...
        if source == "extensions":
            return "navin-extension"
        return f"cli-anything:{source}"

    def _trust_registry(self, app: dict[str, Any]) -> str:
        return "navin-extension" if str(app.get("_source") or "") == "extensions" else "cli-anything"

    def get_app(self, name: str, *, force_refresh: bool = False) -> dict[str, Any]:
        wanted = name.lower()
        if is_china_market_cli_app({"name": wanted}):
        ...
        *,
        ...
        remove = compact_dict({
            ...
            "verification": (
                ...
                else ["navin_state_absent", "managed_paths_absent"]
            ),
        })
        return app_manifest(
            app_id=name,
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from navin."
        return f"""---
...
Use this skill when the user asks navin to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_navin_skill_note(self, content: str, app: dict[str, Any]) -> str:
        marker = "<!-- navin-cli-app-note -->"
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_navin_skill_note(content, app)
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy == "bundled" and still_available:
            message = (
                ...
                "is still available because it is managed outside navin."
            )
