...
"""MCP client: connects to MCP servers and wraps their tools as native navin tools."""
...
def _describe_failure(exc: BaseException) -> str:
    ...
    if isinstance(exc, BaseExceptionGroup):
        return "; ".join(_describe_failure(child) for child in exc.exceptions)[:_MAX_FAILURE_CHARS]
...
class MCPToolWrapper(_MCPWrapperBase):
    ...
    def _store_image_block(
        ...
        from navin.utils.artifacts import ArtifactError, store_generated_image_artifact
...
class MCPResourceWrapper(_MCPWrapperBase):
    """Wraps an MCP resource URI as a read-only navin Tool."""
...
class MCPPromptWrapper(_MCPWrapperBase):
    """Wraps an MCP prompt as a read-only navin Tool."""
...
async def connect_mcp_servers(
    ...
    async def open_single_server(
        name: str, cfg, server_stack: AsyncExitStack
    ...
    async def connect_single_server(name: str, cfg) -> tuple[str, MCPConnection | None]:
        ...
        async def own_connection() -> None:
            try:
                # Own the stack before the handshake: SDK task-group failures
                # cancel initialize(), so cleanup must also cover BaseException.
                # Let __aexit__ unwind in this task and recover the HTTP error
                # before reporting a failed connection to the caller.
                async with AsyncExitStack() as stack:
                    _, connected_stack = await open_single_server(name, cfg, stack)
                    if connected_stack is None:
                        return
                    if not ready.done():
                        ready.set_result(True)
                    await close_requested.wait()
            except Exception as exc:
                logger.warning("MCP server '{}': connection closed: {}", name, _describe_failure(exc))
...
async def reload_servers(state: Any, registry: ToolRegistry) -> dict[str, Any]:
    ...
    async with _reload_lock(state):
        ...
        try:
            ...
            return {
                "ok": False,
                "message": "Could not reload MCP config. Restart navin to pick up changes.",
                "requires_restart": True,
                "error": str(exc),
            }
        ...
        if failed:
            ...
            message = "MCP connections refreshed without restarting navin."
        else:
            message = "MCP config reloaded without restarting navin."
...
async def request_mcp_reload(bus: Any, *, timeout: float = 15.0) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": False,
            "message": "MCP hot reload timed out. Restart navin to pick up changes.",
            "requires_restart": True,
        }
...
async def handle_runtime_control(state: Any, msg: InboundMessage, registry: ToolRegistry) -> bool:
    ...
    try:
        ...
        result = {
            "ok": False,
            "message": "MCP hot reload failed. Restart navin to pick up changes.",
            "requires_restart": True,
            "error": str(exc),
        }
