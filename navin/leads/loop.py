...
registries, hiring signals, open web and OpenStreetMap (rotating cities and
angles every cycle), then watch for tier A, signals and due steps, then the
sequence phase: due steps are drafted in the prospect's language and, only when
the profile is in ``autonomous`` mode, sent within the daily cap and the opt-out
list. In ``approval`` mode nothing leaves the desk. Never scrape LinkedIn people.
...
from navin.leads.heartbeat import profile_is_armed
...
SequenceFn = Callable[..., dict[str, Any]]
...
def _hunt_key(desk: LeadsStore) -> str:
    return str(Path(desk.root).resolve())


def mark_loop_hunting(desk: LeadsStore, live: bool) -> None:
    key = _hunt_key(desk)
    if live:
        _live_hunts.add(key)
    else:
...
def hunt_is_live(desk: LeadsStore, state: dict[str, Any] | None = None) -> bool:
    row = state if isinstance(state, dict) else {}
    if str(row.get("phase") or "") != "hunt":
        return False
    if hunt_expired(row):
...
def recover_stale_hunt(desk: LeadsStore, *, now: float | None = None) -> dict[str, Any]:
    clock = now if now is not None else _now()
    ...
    with leads_desk_lock(desk, wait_s=0) as got:
        if not got:
            return _apply_intent(disk, intent, now=clock)
...
def peek_loop(store: LeadsStore, *, now: float | None = None) -> dict[str, Any]:
    clock = now if now is not None else _now()
...
def _merge_intent(desk: LeadsStore, **fields: Any) -> dict[str, Any]:
...
def _collect_leads(desk: LeadsStore, **_kwargs: Any) -> dict[str, Any]:
    from navin.leads.pipeline import run_hunt

    return run_hunt(desk, source="loop")


def _run_sequences(desk: LeadsStore, **_kwargs: Any) -> dict[str, Any]:
    """Draft due steps; send them only when the profile says autonomous (cap, opt-out)."""
    from navin.leads.pipeline import auto_enroll, run_sequences

    profile = desk.load_profile()
    enrolled = auto_enroll(desk, profile=profile)
    result = run_sequences(desk, profile=profile)
...
def maybe_tick(
    ...
    *,
    ...
    sequence_fn: SequenceFn | None = None,
) -> dict[str, Any]:
    from navin.leads.watch import run_watch

    desk = store or LeadsStore()
    clock = now if now is not None else _now()
    ...
    if not profile_is_armed(profile) and not force:
        return {
            "did_work": False,
            "phase": "unarmed",
            "reason": "leads profile is not set up yet",
            "loop": state,
        }
    ...
    if hunt_is_live(desk, state):
        return _busy_payload(state)
    ...
    with leads_desk_lock(desk, wait_s=0) as got:
        ...
        return _run_hunt(
            ...
            clock=clock,
            force=force,
            collect_fn=collect_fn or _collect_leads,
            watch_fn=watch_fn,
            run_watch=run_watch,
            sequence_fn=sequence_fn or _run_sequences,
        )
...
def _run_hunt(
    ...
    *,
    ...
    # Already holding the desk lock. Do not call peek_loop (recover takes the lock).
    ...
    try:
        try:
            ...
            logger.exception("Leads loop hunt failed")
            try:
        ...
        try:
            watch = call_with_deadline(
                lambda: watch_fn(desk) if watch_fn is not None else run_watch(desk, already_locked=True),
                timeout_s=watch_budget,
                label="watch",
                thread_prefix="navin-leads",
            )
        except Exception as exc:
            logger.exception("Leads loop watch failed")
            watch_error = str(exc)
            watch = {"count": 0, "sent": {}, "error": watch_error}

        sequences: dict[str, Any] = {}
        if sequence_fn is not None:
            remaining = max(
                8.0, MAX_HUNT_S - max(0.0, time.time() - as_float(state.get("hunt_started_at")))
            )
            ...
            try:
                sequences = (
                    call_with_deadline(
                        lambda: sequence_fn(desk),
                        timeout_s=min(SEQUENCE_S, remaining),
                        label="sequence",
                        thread_prefix="navin-leads",
                    )
                    or {}
                ...
                logger.exception("Leads loop sequence phase failed")
                sequences = {"drafted": 0, "sent": 0, "ready": 0, "error": str(exc)}

        added = int((hunt or {}).get("added") or 0)
        scanned = int((hunt or {}).get("scanned") or (hunt or {}).get("found") or 0)
        alerts = int((watch or {}).get("count") or 0)
        sent = int(sequences.get("sent") or 0)
        ready = int(sequences.get("ready") or 0)
        due = next_due_after(state, now=clock, fallback_s=86400)
        cycle = int(state.get("cycle") or 0) + 1
        stamp = format_due(due, _sched(state))
        seq_bit = f", {sent} sent" if sent else (f", {ready} drafts ready" if ready else "")
        summary = f"cycle {cycle}: {added} new leads, {alerts} alerts{seq_bit}, next {stamp}"
        # Stop always wins: _finish_hunt_state reapplies intent after this update.
        state.update(
            {
                "enabled": was_enabled,
                "phase": "idle",
                "cycle": cycle,
                "last_tick": clock,
                "last_watch": clock if not watch_error else state.get("last_watch") or 0,
                "next_due": due,
                "last_result": summary,
...
def start_loop(
    ...
    *,
    ...
    if not profile_is_armed(desk.load_profile()):
        raise LeadsError(
            "Set an ICP on the Leads start screen before starting the loop",
            status=400,
        )
    clock = now if now is not None else _now()
...
def stop_loop(store: LeadsStore | None = None, *, now: float | None = None) -> dict[str, Any]:
    desk = store or LeadsStore()
    clock = now if now is not None else _now()
