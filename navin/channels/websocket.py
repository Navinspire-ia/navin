...
from navin.bus.events import OUTBOUND_META_AGENT_UI, OutboundMessage
...
class WebSocketChannel(BaseChannel):
    ...
    async def _dispatch_terminal_envelope(
        ...
        if t == "terminal_shells":
            await self._send_event(connection, "terminal_shells", shells=available_shells())
            return
        ...
        if t == "terminal_open":
            ...
            async def on_exit(tid: str, exit_code: int | None) -> None:
                self._terminals.discard(connection, tid)
                await self._send_event(
                    connection,
                    "terminal_exit",
                    terminal_id=tid,
