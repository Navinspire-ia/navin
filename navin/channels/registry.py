...
def load_channel_class(module_name: str) -> type[BaseChannel]:
    ...
    from navin.channels.base import BaseChannel as _Base

    mod = importlib.import_module(f"navin.channels.{module_name}")
    for attr in dir(mod):
        obj = getattr(mod, attr)
        if isinstance(obj, type) and issubclass(obj, _Base) and obj is not _Base:
            return obj
    raise ImportError(f"No BaseChannel subclass in navin.channels.{module_name}")
...
def discover_plugins(enabled_names: set[str] | None = None) -> dict[str, type[BaseChannel]]:
    ...
    for ep in entry_points(group="navin.channels"):
