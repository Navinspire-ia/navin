...
def login_github_copilot(
    ...
    client_id = _resolve("NAVIN_GITHUB_COPILOT_CLIENT_ID", GITHUB_COPILOT_CLIENT_ID)
    device_code_url = _resolve("NAVIN_GITHUB_DEVICE_CODE_URL", DEFAULT_GITHUB_DEVICE_CODE_URL)
    access_token_url = _resolve("NAVIN_GITHUB_ACCESS_TOKEN_URL", DEFAULT_GITHUB_ACCESS_TOKEN_URL)
    user_url = _resolve("NAVIN_GITHUB_USER_URL", DEFAULT_GITHUB_USER_URL)
...
class GitHubCopilotProvider(OpenAICompatProvider):
    ...
    def __init__(self, default_model: str = ""):
        ...
        super().__init__(
            api_key="no-key",
            api_base=_resolve("NAVIN_COPILOT_BASE_URL", DEFAULT_COPILOT_BASE_URL),
    ...
    async def _get_copilot_access_token(self) -> str:
        ...
        async with self._copilot_token_lock:
            ...
            if not github_token or not github_token.access:
                raise RuntimeError(
                    "GitHub Copilot is not logged in. Run: navin provider login github-copilot"
                )

            timeout = httpx.Timeout(20.0, connect=20.0)
            async with httpx.AsyncClient(timeout=timeout, follow_redirects=True, trust_env=True) as client:
                response = await client.get(
                    _resolve("NAVIN_COPILOT_TOKEN_URL", DEFAULT_COPILOT_TOKEN_URL),
